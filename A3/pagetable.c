...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
        ...
        if ((coremap[frame].pte->frame & PG_DIRTY) == PG_DIRTY){ //if dirty means we need to put on swap
			evict_dirty_count++;
			coremap[frame].pte->swap_off = swap_pageout(coremap[frame].pte->frame >> PAGE_SHIFT, coremap[frame].pte->swap_off); //into swap file 
			coremap[frame].pte->frame =  coremap[frame].pte->frame | PG_ONSWAP; //has been swapped 
			coremap[frame].pte->frame = coremap[frame].pte->frame & ~PG_DIRTY;
		}
        else if ((coremap[frame].pte->frame & PG_DIRTY) != PG_DIRTY){
			evict_clean_count++;
		}
...
char *find_physpage(addr_t vaddr, char type) {
	...
	temp = (pgtbl_entry_t *)(pgdir[idx].pde & PAGE_MASK);
    p = &temp[idx2];
	...
	if((p->frame & PG_VALID) != PG_VALID){
		...
		coremap[frame].vaddr = vaddr; // For OPT Algorithm
	}
	// Make sure that p is marked valid and referenced. Also mark it
	// dirty if the access type indicates that the page will be written to.
    else if((p->frame & PG_VALID) == PG_VALID){
		hit_count++;
	}
