...
asmlinkage long my_exit_group(struct pt_regs reg)
{
	...
	//deleting pids + need error check or does del_pid error check?
	del_pid(current->pid);
	...
	if (syscall < 0 || syscall > NR_syscalls-1 || syscall == MY_CUSTOM_SYSCALL) {
		...
		if(cmd == REQUEST_SYSCALL_INTERCEPT) {
			...
			if (current_uid() == 0) {
				...
				//check if pid is already intercepted
		...
		//de-intercept syscall arg 
			...
			if (current_uid() == 0) {
				...
				//check if pid is not intercepted
		...
		//start monitoring pid whenever it issues syscall 
			...
			//check if pid is requested by calling process
			...
			//monitor every pid
			//If root is 0 but calling process in not root
			if(current_uid() != 0 && pid == 0) {
				spin_unlock(&my_table_lock);
				return -EPERM;
			} else { //root is 0 and calling process is 0
		...
		//stop monitoring pid, pid = 0 -> all pids
			...
			if(current_uid() != 0 && pid == 0) {
				spin_unlock(&my_table_lock);
				return -EPERM;
			} else { // if root is 0 and process is root
			...
			//check if pid can stop being monitored or is intercepted
			if(table[syscall].monitored == 0 || table[syscall].intercepted == 0) {
				spin_unlock(&my_table_lock);
				return -EINVAL;
			}

			//delete pid from syscall monitored
...
static int init_function(void) {
	...
	//Locking my table + sys call table
	spin_lock(&my_table_lock);
	spin_lock(&sys_call_table_lock);	
	...
	//Hijack + save orig syscall and exit syscall
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
	orig_exit_group = sys_call_table[__NR_exit_group];
...
{   
	...
	//No more monitored pid list + locking my table
	for(j = 0; j<NR_syscalls;j++){
		table[j].monitored = 0;
		table[j].intercepted = 0;
		table[j].listcount = 0;
		destroy_list(j);
	}
